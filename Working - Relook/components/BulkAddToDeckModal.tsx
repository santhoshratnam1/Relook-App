...
interface BulkAddToDeckModalProps {
  decks: Deck[];
  onClose: () => void;
...
const BulkAddToDeckModal: React.FC<BulkAddToDeckModalProps> = ({ decks, onClose, onAdd, onCreateDeck }) => {
  const [isCreatingDeck, setIsCreatingDeck] = useState(false);

  const handleDeckCreated = (deckData: { title: string; description: string }) => {
    const newDeck = onCreateDeck(deckData);
    // Automatically add the selected items to the newly created deck
    onAdd(newDeck.id);
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
            {decks.length > 0 ? decks.map(deck => (
                <button 
                    key={deck.id}
                    onClick={() => { onAdd(deck.id); onClose(); }}
                    className="w-full text-left p-3 rounded-lg bg-[#0C0D0F] hover:bg-white/10 transition-colors flex items-center space-x-3"
                >
...
export default BulkAddToDeckModal;