...
const App: React.FC = () => {
  ...
  const [user, setUser] = useState<User | null>(() => {
    const storedUser = localStorage.getItem('relook-user');
  ...
  useEffect(() => { localStorage.setItem('relook-equipped-items', JSON.stringify(equippedItems));
  ...
  useEffect(() => {
    ...
    if (user && items.length === 0 && !onboardingComplete) {
  ...
  }, [user, missions.length, achievements.length]);
  ...
  }, [undoState, rewards, user, items, decks, achievements, updateMissionProgress, updateRewards]);
  ...
  const handleCreateDeck = useCallback((deckData: { title: string; description: string }) => {
    ...
    return newDeck;
  }, [user, decks, updateRewards, updateMissionProgress]);
  
  const handleLogin = useCallback((loggedInUser: User) => { setUser(loggedInUser); setCurrentPath('/'); setNavHistory(['/']); setRegistrationSuccessMessage(''); }, []);
  ...
  const handleUpdateUser = useCallback((updatedUser: { display_name: string; avatar_url: string; }) => { setUser(prev => prev ? { ...prev, ...updatedUser } : null); }, []);
