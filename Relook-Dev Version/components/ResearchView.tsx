...
const ResearchView: React.FC<ResearchViewProps> = ({ research }) => {
  return (
    <dl>
        ...
        {research.abstract && <p className="text-sm text-gray-400 py-3 px-1">{research.abstract}</p>}
        <InfoRow label="Authors" value={research.authors && research.authors.join(', ')} icon="✍️" />
        ...
        <InfoRow label="Methodology" value={research.methodology} icon="🧪" />
            ...
            value={
                research.keyFindings && research.keyFindings.length > 0 ? (
                    <ul className="list-disc list-inside space-y-1">
                        {research.keyFindings.map((item, index) => <li key={index}>{item}</li>)}
                    </ul>
                ) : undefined
            }
            icon="🎯"
